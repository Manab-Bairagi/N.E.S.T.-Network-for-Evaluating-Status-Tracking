...
![MediaPipe](https://img.shields.io/badge/MediaPipe-0.10%2B-0F9D58?style=for-the-badge&logo=google&logoColor=white)
...
Ollama installed for the LLM query interface
...
NSA Kalman Filter — adapts measurement covariance R inversely to detection confidence. Low-confidence detections (occluded, blurry) are treated with high noise, trusting the motion prediction more.
GMC (Global Motion Compensation) — detects ORB keypoints on a downscaled grayscale frame, matches them across consecutive frames, estimates an affine transform via RANSAC, and compensates all track positions accordingly.
Custom ReID backbone — `ImprovedDeepSortWideResNet` with:
...
Fused cost matrix — matching uses `λ_IoU × IoU_cost + λ_ReID × ReID_cost` (default 0.98 / 0.02), prioritising spatial consistency while using appearance for disambiguation.
---
🎭 Action Classes
